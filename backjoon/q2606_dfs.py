...
def dfs(node, connect_list, visited_list = []):
    print(connect_list)
    for i in connect_list[node-1]:
        # print('***', i)
        if not i in visited_list:
            visited_list.append(i)
            dfs(i, connect_list, visited_list)
        else:
            continue
    
    return visited_list

for one in range(computer_connect):
    n, m = map(int, input().split())
    connect_list[n-1].append(m)

visited_list = dfs(1, connect_list)
